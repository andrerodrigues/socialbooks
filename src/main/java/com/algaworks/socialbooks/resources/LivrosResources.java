...
public class LivrosResources {
	...
	public ResponseEntity<List<Livro>> listar(@PathVariable("nome") String nome){
		CacheControl cacheControl  = CacheControl.maxAge(40, TimeUnit.SECONDS);
		  if(nome.compareToIgnoreCase("''") == 0){
			  nome = "%";
		  }
		return ResponseEntity.status(HttpStatus.OK).cacheControl(cacheControl).body(livrosService.listar(nome));
	}
